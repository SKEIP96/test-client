...
export default function HomePage() {
  ...
  const {
    ...
    loadMore,
    refetch,
  } = useProducts({ q: query, sort, take: 12, inStockOnly });
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card p-6">
        <div className="max-w-2xl space-y-2">
          <h1 className="text-3xl font-semibold tracking-tight">Workshop Supply</h1>
          <p className="text-muted-foreground">
            Tools, materials, and essentials for clean builds — simple checkout, clear stock, no noise.
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-3 md:flex-row md:items-center">
              <Input
                ...
                className="md:w-[520px] md:h-[40px]"
              />
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="in-stock-only" className="text-sm text-muted-foreground">
                  In stock only
                </Label>
              </div>
            </div>
          ...
          <div className="text-sm text-muted-foreground">
            Showing <span className="text-foreground font-medium">{products.length}</span> product(s)
          </div>
        </CardContent>
      </Card>
      ...
      {!isLoading && !error && products.length === 0 && (
        <div className="rounded-lg border bg-card p-6 text-muted-foreground">No products found.</div>
      )}
        ...
        <>
          ...
          <div className="flex justify-center pb-6">
            {canLoadMore ? (
              ...
              <div className="text-xs text-muted-foreground">That’s everything.</div>
            )}
          </div>
        </>
      )}
